...
export default function StudentExamQuestionsPage() {
  ...
  const fetchExamDetails = async () => {
    ...
    try {
      ...
      setExam({
        ...canStartRes.data.exam,
        questions: questionsRes.data.questions,
        results_released: canStartRes.data.exam?.results_released
      });

      // Try fetching existing attempt
      try {
        ...
        startTimer(attemptRes.data.expires_at);
      } catch {
        // No attempt yet → wait for user to click Start Exam
        setTimeLeft(0);
      }
  ...
  const startExam = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Cannot start exam');
    }
    setLoading(false);
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {timeLeft > 0 && currentQuestions.map((mcq, idx) => (
        <Paper key={mcq.id} sx={{ p: 3, mb: 2 }}>
          <Typography variant="h6">Question {indexOfFirstQ + idx + 1}</Typography>
          <Typography><BlockMath math={mcq.text} /></Typography>
          {mcq.image_path && (
            <Box sx={{ my: 2 }}>
              <img
                src={`http://127.0.0.1:5000/uploads/images/${mcq.image_path}`}
                alt="Question"
                loading="lazy"
          ...
          >
            <FormControlLabel value="A" control={<Radio disabled={submitted} />} label={<>A: <InlineMath math={mcq.option_a} /></>} />
            <FormControlLabel value="B" control={<Radio disabled={submitted} />} label={<>B: <InlineMath math={mcq.option_b} /></>} />
            <FormControlLabel value="C" control={<Radio disabled={submitted} />} label={<>C: <InlineMath math={mcq.option_c} /></>} />
            <FormControlLabel value="D" control={<Radio disabled={submitted} />} label={<>D: <InlineMath math={mcq.option_d} /></>} />
          </RadioGroup>
        </Paper>
      ))}

      {timeLeft > 0 && exam.questions?.length > questionsPerPage && (
        <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mb: 2, alignItems: 'center' }}>
          <Button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)}>Prev</Button>
          <Typography>Page {currentPage} of {totalPages}</Typography>
          <Button disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)}>Next</Button>
          <Button variant="contained" onClick={handleSubmitMcqAnswers} disabled={loading} sx={{ ml: 3 }}>
