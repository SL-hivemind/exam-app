...
def register_portal_routes(app, token_required, role_required):
    ...
    def portal_course_detail(course_id):
        ...
        # Check if requester is subscribed
        is_subscribed = False
        token = None
    ...
    def portal_enroll_free(current_user, course_id):
        ...
        sub = CourseSubscription(
            ...
            course_id=course_id,
            status='active' if course.price == 0 else 'pending',
        )
        db.session.add(sub)
        db.session.commit()
    ...
    def portal_start_exam(current_user, content_id):
        ...
        # Check if attempt already exists (one active attempt at a time)
    ...
    def portal_my_subscriptions(current_user):
        ...
        subs = CourseSubscription.query.filter_by(
            public_profile_id=profile.id, status='active'
        ).all()
